...
async function run() {
  ...
  try {
    ...
    console.log('Finding all bookings to recompute total_cost...');
    const res = await client.query(`SELECT b.id, b.slot_id, b.client_id, b.total_cost, s.start_time, s.end_time, u.hourly_rate, u.id AS provider_id FROM bookings b JOIN slots s ON b.slot_id = s.id JOIN users u ON s.provider_id = u.id`);
